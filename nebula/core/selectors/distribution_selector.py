...
class DistributionSelector(Selector):
        ...
        with torch.no_grad():
            for batch_idx, (images, labels) in enumerate(dataloader):
                try:
                    ...
                    logging.exception(f"Error al procesar el batch {batch_idx + 1}: {e}")
                    raise e
    ...
    async def add_embedding(self, node, embedding, embedding_cov, current_neighbors):
        ...
        if len(self.node_embeddings) == len(current_neighbors):
            if self.own_embedding is None:
                logging.info("[DistributionSelector] Own embedding not computed yet")
                while self.own_embedding is None or self.own_embedding_cov is None:
                    logging.info("[DistributionSelector] Waiting for own embedding")
                    await asyncio.sleep(1)
            logging.info("[DistributionSelector] Including own embedding")
            ...
            logging.info("[DistributionSelector] All embeddings received")
            ...
            logging.info("[DistributionSelector] Sorted embeddings")
    ...
    async def get_similarity_embeddings(self, method="cosine"):
        ...
        image_path = os.path.join(
            self.config.participant["tracking_args"]["log_dir"],
            self.config.participant["scenario_args"]["name"],
            f"participant_{self.config.participant['device_args']['idx']}_{method}_similarity_matrix.png",
        )
        ...
        for i, node in enumerate(nodes):
            ...
            similar_nodes_with_scores = [
                f"{sorted_nodes[j]} [{sorted_scores[j]:.4f}]" for j in range(len(sorted_nodes))
            ]
            ...
            if node == own_node:
                ...
                similar_nodes_with_scores = [
                    f"{similar_nodes[j]} [{similar_scores[j]:.4f}]" for j in range(len(similar_nodes))
                ]
                ...
                selected_nodes = {n for n, s in zip(similar_nodes, similar_scores, strict=False) if s >= threshold}
